#!/usr/bin/env node
/**
 * verify_proof.js - Verify ZK proof for packet integrity
 * 
 * Usage: node verify_proof.js <proof_json>
 * 
 * Input: JSON string with proof data
 * Output: JSON with verification result
 */

import { Field, Mina, PublicKey, Signature, Poseidon, UInt64 } from 'o1js';
...
async function verifyProof(proofData) {
  ...
  const radioPacket = new RadioPacket({
    ...
    dataHash: Field(packet.dataHash),
