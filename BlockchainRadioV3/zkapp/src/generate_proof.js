#!/usr/bin/env node
/**
 * generate_proof.js - Generate ZK proof for packet integrity
 * 
 * Usage: node generate_proof.js <packet_json>
 * 
 * Input: JSON string with packet data
 * Output: JSON string with proof
 */

import { Field, Mina, PrivateKey, PublicKey, Signature, Poseidon, UInt64 } from 'o1js';
...
async function generateProof(packetData) {
  ...
  const transaction = await Mina.transaction(deployerAccount, async () => {
    await zkApp.verifyPacketIntegrity(
      packet,
      dataHash,
      senderAccount,
      signature
