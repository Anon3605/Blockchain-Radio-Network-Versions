...
export class RadioPacket extends Struct({
  ...
  static fromData(
    ...
    const dataHash = Poseidon.hash(
      Array.from(dataBytes.slice(0, 31)).map(b => Field(b))
    );
...
export class RadioMeshIntegrity extends SmartContract {
  ...
  @state(Field) lastVerifiedUid = State<Field>();
  
  // Count of successfully verified packets (for statistics)
  @state(Field) totalVerified = State<Field>();
  
  // Root of Merkle tree of authorized senders (future expansion)
  @state(Field) authorizedSendersRoot = State<Field>();
   ...
   */
  @method async verifyPacketIntegrity(
    packet: RadioPacket,
    originalData: Field,
  ...
  @method async quickVerify(
    ...
    const packetHash = packet.hash();
    const signatureValid = senderSignature.verify(
      senderPublicKey,
      [packetHash]
    );
